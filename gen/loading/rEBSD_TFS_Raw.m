function [pat_raw_stack] = rEBSD_TFS_Raw(raw_pats, total_num)
%read patterns from TFS data - will give you back a stack of patterns

% Open the file for reading in binary mode
fileID = fopen(raw_pats, 'rb');

% Read data from the file as unsigned 16-bit integers
% fseek(fileID,[512*512]*2*(pat_num-1),'bof');
data_all = fread(fileID,inf,'uint16');

% Close the file
fclose(fileID);

pat_raw_stack=zeros(549,549,total_num);

s_size=256*256;

parfor n=1:total_num
    ...
    pat_raw_stack(:,:,n)=pat_raw;
end

%cut out the boundary around the edge
pat_raw_stack=pat_raw_stack(36:512,36:512,:);
end