...
[Input_Data.lambda] = calc_lambda(Input_Data.V_in);
%scale the wavelength to the same units as the dhkl
Input_Data.lambda_p=Input_Data.lambda/Input_Data.lattice_param_scale; %lambda normalised by the scale factor of the unit cell

%% Color sets for planes
...
Settings_Cor.gfilt=0; %use a low pass filter
Settings_Cor.gfilt_s=7; %low pass filter sigma
Settings_Cor.radius=0;
Settings_Cor.radius_frac=0.6;  %smaller radius to crop the black portion and to avoid abberation effects on sides 

Settings_Cor.max_var_pc_x=0;
Settings_Cor.max_var_pc_y=0;
Settings_Cor.max_var_pc_z=1;
Settings_Cor.max_var_ori=3*degree;
pattern_crop_factor=4;  % 1 uses orginal resolution, 2 reduces it to half with faster processing

[Input_Data.ECP_Pat,Settings_Cor_out] = EBSP_BGCor( Input_Data.ECP_Pat,Settings_Cor );
Input_Data.Settings_Cor_refine=Settings_Cor_out;
Input_Data.Settings_Cor_refine.radius=1;
Input_Data.Settings_Cor_refine.radius_frac=0.6;
Input_Data.Settings_Cor_refine.resize=1;

pattern_info.size=size(Input_Data.ECP_Pat)/pattern_crop_factor;
pattern_info.size=uint32(pattern_info.size);
Input_Data.Settings_Cor_refine.size=pattern_info.size;
...
if isfield(Input_Data,'ECP_Pat')
    ECP_pattern=Input_Data.ECP_Pat;
    ECP_loaded=1;
end
...
function refine(~,eventdata)
       psize=size(Input_Data.ECP_Pat);
        PatternInfo.ScreenWidth=psize(1);
        PatternInfo.ScreenHeight=psize(2);
        [PatternIn,SettingsCor]=EBSP_BGCor(Input_Data.ECP_Pat,Input_Data.Settings_Cor_refine);
...
%% sub functions
    ...
    function draw_pattern(~,eventdata)
        ...
        % use the stage convention - Rz*Rx*Ry - BUT NOTE THAT THE STAGE USES -Ry
...
%working before
       g_stage=Input_Data.Rz(sZ*pi/180)*Input_Data.Rx(sY*pi/180)*Input_Data.Rz(-sX*pi/180);
        ...
        [EBSD_Geometry ] = EBSP_Gnom( Input_Data,PC_start); %you can change PC_in if you want
...
% Correction for matching patterns with Bruker DynamicS convention (March 11, 2024) for pattern matching in olivine
            ...
            if ~isempty(Family_List)
                for n=1:size(Family_List,1)
                    HKLs=Family_List{n}(:,2:4);
                     ...
                     for p=1:size(HKLs,1)
                         %calculate the bands
                         [bands] = Cone_Build(HKLs(p,:),Crystal_UCell{1}.Astar,Input_Data.lambda_p,RotMat,cone_params);
...
if get(h_toggle_scalebar, 'Value') == 1
 ...
 f1(3) = scatter(0, 0, 150, 'kx', 'Parent', h_ecp);
        ...
        if isfield(Input_Data,'crystal_shape')
            ...
            h_pf1 = axes('Units','normalized','Position',[0.2 0.8817 0.1 0.1],'Parent',f);
            h_pf2 = axes('Units','normalized','Position',[0.3 0.8817 0.1 0.1],'Parent',f);
            ...
            try
                ...
                % plot(1,1,ori_single * cS * 4,'Parent',h_cry);
              % set(gcf, 'color', 'none');    
